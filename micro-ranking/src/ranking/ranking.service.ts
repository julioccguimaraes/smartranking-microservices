...
export class RankingService {
	...
	async getRankings(
		...
		try {
			this.logger.log(
				`idCategory: ${idCategory} dataChallenger: ${dataChallenger}`
			);

			if (!dataChallenger) {
				dataChallenger = momentTimezone()
			...
			_.remove(rankingRegisters, function (item) {
				return (
					challenges.filter(
						(challenge) => challenge._id == item.challenge
					).length == 0
				);
			});

			this.logger.log(
				`rankingRegisters: ${JSON.stringify(rankingRegisters)}`
			);

			//Agrupar por player

			const resultado = _(rankingRegisters)
				.groupBy('player')
				.map((items, key) => ({
					player: key,
			...
			const orderedResult = _.orderBy(resultado, 'points', 'desc');
			...
			orderedResult.map(function (item, index) {
				...
				history.victories = item.history.VITORIA
					? item.history.VITORIA
					: 0;
				history.defeats = item.history.DERROTA
					? item.history.DERROTA
					: 0;
