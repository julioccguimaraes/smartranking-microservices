...
export class ChallengeController {
	...
	async getChallenges(
		...
		try {
			...
			if (idPlayer) {
				return await this.challengeService.getPlayerChallenges(
					idPlayer
				);
			} else if (_id) {
	...
	async getChallengesCompleted(
		...
		try {
			...
			if (dataChallenger) {
				return await this.challengeService.getChallengesCompletedByDate(
					idCategory,
					dataChallenger
				);
			} else {
				return await this.challengeService.getChallengesCompleted(
					idCategory
				);
			}
		} finally {
	...
	async updateMatchChallenge(
		...
		try {
			...
			await this.challengeService.updateMatchChallenge(
				idMatch,
				challenge
			);
			await channel.ack(originalMsg);
