    ...
    */
    ...
    async addChallenge(@Body() addChallengeDto: AddChallengeDto) {
        ...
        /*
            ...
            do desafio
        */
        const jogadores: Player[] = await this.clientAdminBackend.send('get-players', '').toPromise()

        addChallengeDto.players.map(playerDto => {
            const playerFilter: Player[] = jogadores.filter(jogador => jogador._id == playerDto._id)
        ...
        */
        const requesterIsPlayerOfTheMatch: Player[] = addChallengeDto.players.filter(player => player._id == addChallengeDto.requester)

        this.logger.log(`requesterIsPlayerOfTheMatch: ${JSON.stringify(requesterIsPlayerOfTheMatch)}`)
        ...
        */
        const category = await this.clientAdminBackend.send('get-categories', addChallengeDto.category).toPromise()
    ...
    async getChallenges(@Query('idPlayer') idPlayer: string): Promise<any> {
        ...
        */
        return this.clientChallenge.send('get-challenges', { idPlayer: idPlayer, _id: '' }).toPromise()
    }
    ...
    async updateChallenge(
        ...
        */
        ...
        this.clientChallenge.emit('update-challenge', { id: _id, challenge: updateChallengeDto })
    }
